...
export default function App() {
  ...
  const { productos, agregar: agregarProd, actualizar: actualizarProd, eliminar: eliminarProd, reordenar: reordenarProd } = useProductos()
  ...
  const { comparativos, guardar: guardarComparativo, actualizar: actualizarComparativo, eliminar: eliminarComparativo } = useComparativos()
      ...
      <aside className="hidden lg:flex w-[220px] bg-[#0A0A0A] border-r border-white/[0.07] flex-col p-5 shrink-0 h-screen sticky top-0">
        ...
        <div className="mt-auto pt-4 border-t border-white/[0.07]">
          <div className="flex items-center gap-2.5 mb-3">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-xs text-white/80 font-medium truncate">{negocioActivoInfo?.nombre ?? nombre}</p>
      ...
      <main className="flex-1 p-4 lg:p-6 pt-16 lg:pt-6 pb-20 lg:pb-6 overflow-auto">
        ...
        {canAdminViews && view === 'liquidacion' && (
          <Liquidacion
            ...
            guardarComparativo={guardarComparativo} actualizarComparativo={actualizarComparativo} eliminarComparativo={eliminarComparativo}
          />
        )}
...
function SidebarLink({ label, active, onClick, badge }: { label: string; active: boolean; onClick: () => void; badge?: number }) {
  return (
    <button onClick={onClick}
      className={`text-left text-sm px-3 py-2 rounded-lg transition-all flex items-center justify-between ${active ? 'bg-white/10 text-white font-medium' : 'text-white/45 hover:text-white/70 hover:bg-white/5'
        }`}>
      {label}
...
function MobileMenuItem({ label, active, onClick, icon, badge }: { label: string; active: boolean; onClick: () => void; icon: React.ReactNode; badge?: number }) {
  return (
    <button onClick={onClick}
      className={`w-full flex items-center gap-3.5 px-4 py-3 rounded-xl transition-all ${active ? 'bg-[#CDA52F]/10 text-[#CDA52F]' : 'text-white/60 active:bg-white/5'
        }`}>
      {icon}
